...
#### 如何实现一个靠谱的协议  
`TCP` 协议使用为了保证顺序性 每个包都有一个ID  
在建立连接的时候 会商定起始ID是什么 然后按照ID 一个个发送  
为了保证不丢包 对于发送的包都要进行应答  
这个应答也不是一个一个来 而是应答某个之前的ID  表示已经收到了
这种模式称为 `累计确认` 或者 `累计应答`  （cumulative acknowledgement）  
为了记录所以发送的包和接受的包 `TCP` 也需要发送端和接收端分别都有缓存来保存这些记录  
- 发送了并且已经确认
- 发送了但尚未确认 
- 没有发送但等待发送 
- 没有发送暂时也不会发送 
...
##### 顺序问题与丢包问题  
丢包问题 一种方法就是 `超时重试` 对每一个发送了但是没有回复的包都设有一个定时器  
超过了一定时间就尝试重新发送  `TCP` 会通过采样 `RTT` 时间进行加权平均算出一个值  
而且这个值还是不断变化的 这个算法 我们称之为 `自适应重传算法` (adaptive retransmission algorithm)  
如果再次超时需要重传的时候  `超时间隔加班 每当遇到一次超时重传的时候 都会将下一次超时时间间隔设为  
前面的两倍 说明网络环境差 不宜频繁反复发送`
  
##### 流量控制问题与拥塞控制问题
流量控制都是通过滑动窗口来解决的    
这其实就相当于你领导和你的工作备忘录 布置过的工作要有编号 干完了有反馈活不能派太多也不能太少    
